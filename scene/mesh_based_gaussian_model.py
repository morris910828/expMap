...
class MeshBasedGaussianModel:
    ...
    def get_xyz(self):
        if self._bc is not None:
            ...
            return torch.cat((self._xyz, added_xyz), dim=0)
        return self._xyz
        
    @property
    def get_added_xyz(self):
        bc = self.bc_activation(self._bc,dim=1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float):
        ...
        triangles = self.vertices[self.face_idx].float()
        vertex1 = triangles[:, 0, :].float().cuda() # [n, 3]
        vertex2 = triangles[:, 1, :].float().cuda() # [n, 3]
        vertex3 = triangles[:, 2, :].float().cuda() # [n, 3]

        a = torch.norm(vertex1 - vertex2 , dim=1, keepdim=True)
        b = torch.norm(vertex2 - vertex3, dim=1, keepdim=True)
        c = torch.norm(vertex3 - vertex1, dim=1, keepdim=True)
        self.r = (a + b + c) / 3
    ...
    def save_geo_ply(self, path):
        ...
        faces_np = self.face_idx.detach().cpu().numpy()

        face_dtype_full = [(attribute, 'f4') for attribute in ['fa', 'fb', 'fc']]
        face_elements = np.empty(self.face_idx.shape[0], dtype=face_dtype_full)
        face_attributes = faces_np
        face_elements[:] = list(map(tuple, face_attributes))
    ...
    def save_app_ply(self, path):
        ...
        xyz = self.get_added_xyz.detach().cpu().numpy()
        ...
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, bc, d, fid, f_dc, f_rest, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def load_geo_ply(self, path, use_train_test_exp = False):
        ...
        faces = np.stack((np.asarray(plydata.elements[1]["fa"]),
                          np.asarray(plydata.elements[1]["fb"]),
                          np.asarray(plydata.elements[1]["fc"])),  axis=1)
    ...
    def load_app_ply(self, path, use_train_test_exp = False):
        ...
        self.fid = torch.tensor(fid, dtype=torch.long, device="cuda")
    ...
    def densify_and_split_init(self, grads, grad_threshold, scene_extent, N=2):
        ...
        num_vertices = selected_pts_mask.shape[0]
        vertex_face_count = torch.bincount(self.face_idx.flatten(), minlength=num_vertices)

        # 取出 mask == True 的頂點的面數
        masked_face_counts = vertex_face_count[selected_pts_mask].unsqueeze(1) 

        with torch.no_grad():
            self._scaling[selected_pts_mask] = self.scaling_inverse_activation(
                self.get_scaling[selected_pts_mask] / (masked_face_counts * 0.8)
            )

        selected_fid = self.split_neighbor_gaussians(selected_pts_mask)
        selected_faces = self.face_idx[selected_fid]
        self._bc = torch.ones_like(selected_faces).float().cuda() / 3
        self._distance = torch.zeros_like(selected_fid.unsqueeze(1)).float().cuda()
        self.fid = selected_fid.unsqueeze(1).long().cuda()
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii, vertex_split=True):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_split(grads, max_grad, extent, vertex_split=vertex_split)

        torch.cuda.empty_cache()

    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, vertex_split=True):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
        ...
        if vertex_split:
            ...
            self.densification_postfix(new_bc, new_distance, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation)

        self.fid = torch.cat((self.fid, new_added_fid), dim=0)
        self.densification_postfix(new_added_bc, new_added_distance, new_added_features_dc, new_added_features_rest, new_added_opacity, new_added_scaling, new_added_rotation)
    ...
    def densification_postfix(self, new_bc, new_distance, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"bc": new_bc,
