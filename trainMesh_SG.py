...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = StructuralGaussianModel(dataset.sh_degree, opt.optimizer_type, mesh_path)
    scene = Scene(dataset, gaussians)
    gaussians.training_s1_setup(opt)
    if checkpoint:
    ...
    depth_l1_weight = get_expon_lr_func(opt.depth_l1_weight_init, opt.depth_l1_weight_final, max_steps=7000)
    ...
    # Stage 1
    progress_bar = tqdm(range(first_iter, second_iter), desc="Stage 1 Training progress")
    first_iter += 1
    for iteration in range(first_iter, second_iter + 1):
        ...
        scale_loss = torch.mean(gaussians.get_scaling ** 2)

        lambda_scale = 0.01  
        scale_loss_weak = lambda_scale * torch.mean(gaussians.get_scaling ** 2)

        vertex_radius = gaussians.get_vertex_radius
        vertex_mr_loss = mesh_vertex_restrict_loss(gaussians.get_geo_scaling, vertex_radius, weight=1.0)

        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value) + scale_loss_weak
    ...
    # Generate appearance gaussians
    gaussians.max_radii2D[visibility_filter] = torch.max(gaussians.max_radii2D[visibility_filter], radii[visibility_filter])
    gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter)

    size_threshold = 20 
    gaussians.densify_and_prune_init(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold, radii)

    gaussians.training_s2_setup(opt)
    # Stage 2
    progress_bar = tqdm(range(second_iter, third_iter), desc="Stage 2 Training progress")
    second_iter += 1
    for iteration in range(second_iter, third_iter + 1):
        ...
        #target = torch.mean(gaussians.get_scaling.detach())
        #scale_loss = torch.mean((gaussians.get_scaling - target) ** 2)
        scale_loss = torch.mean(gaussians.get_scaling ** 2)

        vertex_radius = gaussians.get_vertex_radius
        vertex_mr_loss = mesh_vertex_restrict_loss(gaussians.get_geo_scaling, vertex_radius, weight=0.5)

        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value) + aniso_loss
        ...
        loss.backward()
        #gaussians.weight_control_optimizer()
    ...
    for iteration in range(third_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Densification
            if iteration < opt.densify_until_iter_sg3:
                # Keep track of max radii in image-space for pruning
                gaussians.max_radii2D[visibility_filter] = torch.max(gaussians.max_radii2D[visibility_filter], radii[visibility_filter])
                gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter)
