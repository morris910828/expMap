...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, train_test_exp, separate_sh, render_path, frame_idx):
    
    for idx, view in enumerate(tqdm(views, desc="Rendering frame" f"{frame_idx}")):
...
def deformation(dataset : ModelParams, iteration : int, pipeline : PipelineParams, deform_path : str, separate_sh: bool):
    with torch.no_grad():
        ...
        print("Numbers of frames:", frames.shape[0])

        for frame_idx, deform_vertices in enumerate(frames):

            deform_vertices = torch.from_numpy(deform_vertices).float().to("cuda")
            update_gaussians_MBG(gaussians, verties, deform_vertices, faces)

            bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
            background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

            render_set(dataset.model_path, name, scene.loaded_iter, scene.getValCameras(), gaussians, pipeline, background, dataset.train_test_exp, separate_sh, render_path, frame_idx)
